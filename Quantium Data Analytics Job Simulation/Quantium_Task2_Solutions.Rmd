...
control_store <- score_Control[Store1 != Store2,
                               Store2[which.max(finalControlScore)]]
control_store
...
percentageDiff <- merge(scaledControlSales,
                        ...
                        by = "YEARMONTH"
                        )[, percentageDiff := abs(controlSales - totSales.y)/controlSales]
                        #)[, percentageDiff := abs(controlSales - totSales.y) / ((controlSales + totSales.y) / 2) * 100]
...
#### the data table.
pastSales <- measureOverTimeSales[, Store_type := Store_type
 ][, totSales := mean(totSales), by = c("YEARMONTH","Store_type")
...
percentageDiff <- merge(scaledControlCustomers,
                        ...
                        by = "YEARMONTH"
                        )[, percentageDiff := abs(controlCustomers - nCustomers.y)/controlCustomers]
```
...
percentageDiff <- merge(scaledControlSales,
                        measureOverTimeSales[STORE_NBR == trial_store, ],
                        by = "YEARMONTH")[, percentageDiff := abs(
                          controlSales - totSales.y)/controlSales]
